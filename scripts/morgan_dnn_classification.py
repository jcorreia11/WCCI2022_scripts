...
def baseline_dnn_model(n_hidden_layers = 3,
...
def train_morgan_dnn_classification(output_path = 'outputs/multiclass_dnn_morgan_metainter_retrorules/',
    ...
    estimator.model.save(folder + '/trained_model_dnn_morgan_class.h5')

    estimator.model.save_weights(folder + '/trained_model_dnn_morgan_weights_class.h5')
    ...
    plt.savefig(folder + '/accuracy_history_dnn_morgan_class.png')
    plt.close()
    ...
    plt.legend(['train', 'val'], loc='upper left')
    plt.show()
    plt.savefig(folder + '/loss_history_dnn_morgan_class.png')
    plt.close()
